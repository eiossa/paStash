...
function WhoIsPublishing(room){
 ...
 db.search([{subject: db.v('x'), predicated: "publisher", object: room}],function(err,results){
  if (err)
	console.log(err);
  else { console.log(results);
	/*for (i=0;i<solutions.length;i++)
	  console.log("publisher numero ",i,": ",solutions[i].x);
...
//	this.db = gun();
	...
	db = levelgraph(level("yourdb632478"));
...
//		per il momento non fare nulla
	...
	if (obj.event.opaque_id && obj.event.name=='attached'){
		...
		triples = [{subject: obj.session_id, predicate:'created', object: obj.event.opaque_id }]; //TODO così lo crea ogni volta?
		...
		/*query = { subject: obj.event.opaque_id }	//stampa tutti gli handle associati all'opaque
...
// TODO provare a cercare per displayname)
	...
	if(obj.event.data.event == "joined" ){		//crea tripla OpaqueID -> publishingID -> Room (displayname)
		...
		db.search([{subject: x, object: obj.handle_id}], function (err,solutions){ 
			...
			try { 
			     console.log("TRIPLA INSERITA: ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object,"  ",triples[0].displayname);	
			     db.put(triples);
			}
		...
		db.search([{subject: x, object: obj.handle_id}], function (err,solutions){ 
			...
			try { 
			     console.log("TRIPLA INSERITA: ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object);	     				     db.put(triples);
			    WhoIsPublishing();
			}
			catch (err) {console.log(err)}
		});		
...
//TODO eliminare i subscribers
		...
		db.search([{subject: x, predicate: "related to", object: obj.handle_id }], function (err,solutions){ 
			triples = [{subject: solutions[0].x, predicate: "publisher", object: obj.event.data.room}];
			try { 
			     console.log("TRIPLA ELIMINATA: ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object);	     				     db.del(triples);
			     WhoIsPublishing();
			}
			catch (err) {console.log(err)}
		});
