...
// TODO if plugin is videoroom
    ...
    //l'user proprietario dell'handle è entrato nella room obj.event.data.room con displayname obj.event.data.display
      ...
      //crea tripla OpaqueID -> publishingID -> Room (displayname)
		...
		//l'user con publisher id = obj.event.data.id sta pubblicando (è un publisher) nella room obj.event.data.room tramite l'handle = handle_id

		// crea tripla OpaqueID -> "publisher" -> Room (handle_id)
			db.search([{subject: x=db.v('x'), predicate: obj.session_id, object: obj.handle_id}], function (err,solutions){
			triples = [{subject: solutions[0].x, predicate: "publisher", object: obj.event.data.room, "handle":obj.handle_id}];
			try {
          db.put(triples, function(err){
			    WhoIsPublishing();
        });
