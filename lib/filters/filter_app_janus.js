...
bob.path('spouse').put(alice);*/
	...
	if (obj.event.opaque_id && obj.event.name!='detached'){
		//console.log("\nTIPO 2: ",obj);
	 	session.get(obj.session_id).put({opaque_id: obj.event.opaque_id},function(){});

		console.log("\nOPAQUE DEL MESSAGGIO: ", obj.event.opaque_id);		
		session.get(obj.session_id).val(function(opaque_id){		//STAMPA L'INTERO OGGETTO, come si fa ad avere solo l'id?
		console.log('\nOPAQUE: ',opaque_id)	
		});

		console.log("\nHANDLE DEL MESSAGGIO: ", obj.handle_id);
		handle.get(obj.session_id).put({handle_id: obj.handle_id});
		handle.get(obj.session_id).val(function(handle_id){
		console.log('\nHANDLE: ',handle_id)
});
		//console.log("\nH : ",h);

		//inserisci un puntatore all'handle corrispondente in session -> opaque_id -> handle
//	 	session.get(obj.session_id).get(obj.event.opaque_id).put('handle');		// prova a stampare handle e poi
//		session.get(obj.session_id).get(obj.event.opaque_id).get('handle').put(h);	// il livello sottostante
		
		//funzione di stampa del puntatore inserito
		//se il file di gun è stato già creato non funziona(handle undefined)   -> perchè ????
		/*var hlist = session.get(obj.session_id).get(obj.event.opaque_id).get('handle');
		hlist.val(function(opaque){
