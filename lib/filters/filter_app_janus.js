...
////chi è sottoscritto nella stanza
function WhoIsSubscribed(){
 room = 1234;
...
// session create/destroy
	 ...
	 //store opaqueID and handleID
	  if (obj.event.opaque_id && obj.event.name=='attached'){
	   ...
	   /*	triples = [{subject: obj.session_id, predicate:'created', object: obj.event.opaque_id }];
        ...
        //Creo tripla Opaque_ID -> related to -> Handle_ID
        //TODO modificare in Session -> OPAQUE -> HANDLE
    ...
    //cerca il soggetto (opaque_ID) relativo all'handle detached
    //elimina la tripla Opaque_ID -> related to -> Handle_ID
    //TODO riscrivere con la GET, non mi serve search (GET la tripla con handle_ID e cancellala)
		db.search([{subject: db.v('x'), object: obj.handle_id}], function (err,solutions){
			if(err)
			   console.log(err);
      else {
        triples = {subject:solutions[0].x,predicate:"related to",object:obj.handle_id};
...
// TODO provare a cercare per displayname
	...
	if(obj.event.data.event == "joined" ){
	   ...
	   // cerca l'opaque associato all'handle
		db.search([{subject: x=db.v('x'), object: obj.handle_id}], function (err,solutions){
      //crea tripla OpaqueID -> publishingID -> Room (displayname)
			...
			try {
			   //  console.log("TRIPLA JOIN INSERITA: ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object,"  ",triples[0].displayname);
			    db.put(triples,function(err){ // è questa che mi fa uscire due risultati da WhoIsPublishing
        });
		...
		// crea tripla OpaqueID -> "publisher" -> Room
			...
			try {
			 //    console.log("TRIPLA PUBLISHED INSERITA: ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object);
          db.put(triples);
			    WhoIsPublishing();
			}
			catch (err) {console.log(err)}
...
//elimina gli utenti sottoscritti a all'utente (OpaqueID -> "subscribed" -> OpaqueID)
    ...
    //cerco il "proprietario" dell'handle del messaggio e cancello la tripla Opaque -> publisher -> room
		db.search([{subject: db.v('x'), predicate: "related to", object: obj.handle_id }], function (err,solutions){
			triples = [{subject: solutions[0].x, predicate: "publisher", object: obj.event.data.room}];
			try {
			     //console.log("TRIPLA ELIMINATA (unpublished): ",triples[0].subject,"  ", triples[0].predicate,"  ", triples[0].object);
           ...
           db.get({predicate:"subscribed",object:solutions[0].x},function(err,list){
                  for(i=0;i<list.length;i++){
                    //  console.log("view numero ",i," ",list[i])
    ...
    //cerco l'utente "proprietario" dell'handle contenuto nel messaggio e l'utente a cui si sta sottoscrivendo (tramite publishing id e romm)
    db.search([{subject: db.v('x'), predicate: "related to", object: obj.handle_id},
				{subject: db.v('y'), predicate: obj.event.data.feed, object: obj.event.data.room}],function(err,solutions){
